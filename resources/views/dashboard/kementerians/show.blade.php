    ...
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse px-4 lg:px-12">
            ...
            <div class="flex items-center">
                ...
                <a href="/dashboard/kastrats/{{ $kementerian->name }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">
                    {{ $kementerian->name }}
                </a>
            </div>
            </li>
    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
            <div style="max-height: 350px; overflow:hidden">
                <img src="{{ asset('storage/' . $kementerian->image) }}" alt="{{ $kementerian->name }}" class="img-fluid mt-3">
            </div>
            <h2 class="mb-2 text-xl font-semibold leading-none text-gray-900 md:text-2xl dark:text-white">{{ $kementerian->name }}</h2>
            <dl>
                <dt class="mb-2 font-semibold leading-none text-gray-900 dark:text-white">Body</dt>
                <dd class="mb-4 font-light text-gray-500 sm:mb-5 dark:text-gray-400">{{ $kementerian->description }}</dd>
            </dl>
            <div class="flex items-center space-x-4">
                <a href="/dashboard/kementerians/{{ $kementerian->id }}/edit" class="text-white inline-flex items-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
